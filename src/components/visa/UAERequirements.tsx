...
import { getUAERequirementsByCategory, getAllUAECategories, RequirementItem } from '@/utils/uaeVisaRequirements';

interface UAERequirementsProps {
...
const UAERequirements = ({ selectedCategory = 'Tourist-30', onCategoryChange }: UAERequirementsProps) => {
  const categories = getAllUAECategories();
  const requirements = getUAERequirementsByCategory(selectedCategory);
    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-gray-600 mb-8">
          Requirements vary by visa type and duration. Select your category below to see specific requirements.
        </p>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-blue-50 border border-teal-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <div className="font-bold text-teal-600">{requirements.processingTime}</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-sm text-gray-600">Stay Duration</div>
            <div className="font-bold text-teal-600">{requirements.stayDuration}</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-sm text-gray-600">Visa Fee</div>
            <div className="font-bold text-teal-600">{requirements.fee}</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        <h4 className="font-bold text-yellow-800 mb-2 flex items-center gap-2">
          ...
          Important Notes for UAE Visa
        </h4>
        <ul className="text-sm text-yellow-700 space-y-1">
          <li>• UAE visa on arrival available for some nationalities</li>
          <li>• All visitors must have health insurance coverage</li>
          <li>• Hotel booking must be confirmed before visa approval</li>
          <li>• Overstaying results in daily fines and potential ban</li>
          <li>• Visa requirements may change based on nationality</li>
        </ul>
      </div>
    </div>
  );
...
export default UAERequirements;
