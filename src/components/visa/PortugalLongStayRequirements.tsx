...
import { getPortugalLongStayRequirementsByCategory, getAllPortugalLongStayCategories, RequirementItem } from '@/utils/portugalLongStayRequirements';

interface PortugalLongStayRequirementsProps {
...
const PortugalLongStayRequirements = ({ selectedCategory = 'Work', onCategoryChange }: PortugalLongStayRequirementsProps) => {
  const categories = getAllPortugalLongStayCategories();
  const requirements = getPortugalLongStayRequirementsByCategory(selectedCategory);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-green-50 to-white border border-green-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <div className="font-bold text-green-600">{requirements.processingTime}</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-sm text-gray-600">Stay Duration</div>
            <div className="font-bold text-green-600">{requirements.stayDuration}</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-sm text-gray-600">Visa Fee</div>
            <div className="font-bold text-green-600">{requirements.fee}</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        <h4 className="font-bold text-yellow-800 mb-2 flex items-center gap-2">
          ...
          Important Notes for Portugal Long-Stay Visa
        </h4>
        <ul className="text-sm text-yellow-700 space-y-1">
          <li>• All documents must be apostilled or legalized</li>
          <li>• Health insurance must include emergency repatriation</li>
          <li>• D7 visa requires proof of passive income, not employment</li>
          <li>• Processing times may vary by consulate</li>
          <li>• Residence permits are renewable upon meeting conditions</li>
        </ul>
      </div>
    </div>
  );
...
export default PortugalLongStayRequirements;
