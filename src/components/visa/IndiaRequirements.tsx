...
import { getIndiaRequirementsByCategory, getAllIndiaCategories, RequirementItem } from '@/utils/indiaVisaRequirements';

interface IndiaRequirementsProps {
...
const IndiaRequirements = ({ selectedCategory = 'e-Tourist', onCategoryChange }: IndiaRequirementsProps) => {
  const categories = getAllIndiaCategories();
  const requirements = getIndiaRequirementsByCategory(selectedCategory);
    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-gray-600 mb-8">
          Requirements vary by e-Visa type. Select your category below to see specific requirements.
        </p>
      ...
      <div className="bg-gradient-to-r from-orange-50 to-green-50 border border-orange-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <div className="font-bold text-orange-600">{requirements.processingTime}</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-sm text-gray-600">Stay Duration</div>
            <div className="font-bold text-orange-600">{requirements.stayDuration}</div>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <div className="text-sm text-gray-600">Visa Fee</div>
            <div className="font-bold text-orange-600">{requirements.fee}</div>
          </div>
        </div>
      </div>
      ...
      <div>
        <h3 className="text-xl font-bold mb-6 text-gray-900 text-center">
          Standard Requirements (All e-Visa Types)
        </h3>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        <h4 className="font-bold text-yellow-800 mb-2 flex items-center gap-2">
          ...
          Important Notes for India e-Visa
        </h4>
        <ul className="text-sm text-yellow-700 space-y-1">
          <li>• e-Visa must be applied 4 days before travel date</li>
          <li>• Entry only through designated airports and seaports</li>
          <li>• Biometric data will be collected on arrival</li>
          <li>• Cannot be extended or converted to other visa types</li>
          <li>• Valid for tourism, business, medical, or conference purposes only</li>
        </ul>
      </div>
    </div>
  );
...
export default IndiaRequirements;
